    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            with torch.cuda.amp.autocast(fp16_scaler is not None):
                image_features = backbone.encode_image(images, ifget = True).float()
                text_features = model_tes(images)
                text_features_ln = ln_t(text_features)


                ## -----text_branch-----
                student_proj_text, student_out_text,_= projector(text_features_ln)
                ...
                ## -----visual_branch-----
                student_proj, student_out,kl_loss,con_loss,student_cls= projector(image_features,if_images=True,if_tar=True,if_compute_loss=True)
                ...
                anchor_img = [student_cls[mask_lab_2][torch.where(class_labels_2[mask_lab_2] == i)[0]].mean(0) \
                    for i in lab_idx]
                anchor_img = torch.vstack(anchor_img).detach()
                
                
                sim_img = F.softmax(F.normalize(student_cls,dim=-1) @ F.normalize(anchor_img,dim=-1).t() / 0.4, dim=1)
                ...
                # pstr += f'con_loss: {con_loss.item():.4f} '

                loss = loss_v + loss_t + args.lamC * loss_cico + 0.001 * kl_loss
...
def test(backbone, projector, model_tes,ln_t, test_loader, epoch, save_name, args,device):
    ...
    for batch_idx, (images, label, _) in enumerate(tqdm(test_loader)):
        ...
        with torch.no_grad():
            ...
            text_features = model_tes(images)
            text_features_ln = ln_t(text_features)
            _, logits_text,_= projector(text_features_ln)
            _, logits_img,_,_,_= projector(image_features,if_images=True,if_tar=True,if_compute_loss=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--TES_model_dir', type=str, default="/home/yistar/Downloads/TAR/dev_outputs/GET/log/TES_CUB_TAR_GET_CUB_TAR_05MASK_20260126_103220/checkpoints/tes_model.pt")
    ...
    init_experiment(args, runner_name=['TAR'], exp_id=args.exp_id)
    ...
    # PROJECTION HEAD
    # ----------------------
    projector = DINOHead(in_dim=768, out_dim=args.mlp_out_dim, nlayers=args.num_mlp_layers,trm_top_m_ratio=args.top_m_ratio).to(device)
